...
/**
 * This class is a wrapper for the TA-Lib function CDLTASUKIGAP: Tasuki Gap.
 */
public class CdlTasukiGap {

    private static final Logger logger = LoggerFactory.getLogger(CdlTasukiGap.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLTASUKIGAP(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLTASUKIGAP returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLTASUKIGAP returned error code: " + retCode);
        }
