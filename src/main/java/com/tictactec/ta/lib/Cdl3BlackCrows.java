...
public class Cdl3BlackCrows {
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDL3BLACKCROWS(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDL3BLACKCROWS returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDL3BLACKCROWS returned error code: " + retCode);
        }
