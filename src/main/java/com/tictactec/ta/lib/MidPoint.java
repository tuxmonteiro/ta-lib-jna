...
/**
 * This class is a wrapper for the TA-Lib function MIDPOINT: MidPoint over period.
 */
public class MidPoint {

    private static final Logger logger = LoggerFactory.getLogger(MidPoint.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_MIDPOINT(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function MIDPOINT returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function MIDPOINT returned error code: " + retCode);
        }
