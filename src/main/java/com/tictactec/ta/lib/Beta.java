...
/**
 * This class is a wrapper for the TA-Lib function BETA: Beta.
 */
public class Beta {
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal0, double[] inreal1, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_BETA(startIdx, endIdx, inreal0, inreal1, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function BETA returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function BETA returned error code: " + retCode);
        }
