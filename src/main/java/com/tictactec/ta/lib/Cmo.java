...
/**
 * This class is a wrapper for the TA-Lib function CMO: Chande Momentum Oscillator.
 */
public class Cmo {

    private static final Logger logger = LoggerFactory.getLogger(Cmo.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CMO(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function CMO returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CMO returned error code: " + retCode);
        }
