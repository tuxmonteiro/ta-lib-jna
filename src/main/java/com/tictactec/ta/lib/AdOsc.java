...
public class AdOsc {
    ...
    public static Result execute(int startIdx, int endIdx, double[] high, double[] low, double[] close, double[] volume, int optInFastPeriod, int optInSlowPeriod) {
        Result result = new Result();
        IntByReference outBegIdx = new IntByReference();
        IntByReference outNBElement = new IntByReference();
        int allocationSize = high.length;
        double[] outReal = new double[allocationSize];
        int retCode = taLib.TA_ADOSC(startIdx, endIdx, high, low, close, volume, optInFastPeriod, optInSlowPeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
