...
/**
 * This class is a wrapper for the TA-Lib function DX: Directional Movement Index.
 */
public class Dx {

    private static final Logger logger = LoggerFactory.getLogger(Dx.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] high, double[] low, double[] close, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_DX(startIdx, endIdx, high, low, close, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function DX returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function DX returned error code: " + retCode);
        }
