...
/**
 * This class is a wrapper for the TA-Lib function AVGDEV: Average Deviation.
 */
public class AvgDev {

    private static final Logger logger = LoggerFactory.getLogger(AvgDev.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_AVGDEV(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function AVGDEV returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function AVGDEV returned error code: " + retCode);
        }
