...
/**
 * This class is a wrapper for the TA-Lib function COS: Vector Trigonometric Cos.
 */
public class Cos {
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_COS(startIdx, endIdx, inreal, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function COS returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function COS returned error code: " + retCode);
        }
