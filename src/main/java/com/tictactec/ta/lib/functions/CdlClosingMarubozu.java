...
/**
 * This class is a wrapper for the TA-Lib function CDLCLOSINGMARUBOZU: Closing Marubozu.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlClosingMarubozu {

    private static final Logger logger = LoggerFactory.getLogger(CdlClosingMarubozu.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlClosingMarubozu() {
    }

    /**
     * Calculates the Closing Marubozu pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Closing Marubozu pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLCLOSINGMARUBOZU(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLCLOSINGMARUBOZU returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLCLOSINGMARUBOZU returned error code: " + retCode);
        }
