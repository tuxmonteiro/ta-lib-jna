...
/**
 * This class is a wrapper for the TA-Lib function CDLCONCEALBABYSWALL: Concealing Baby Swallow.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlConcealBabysWall {

    private static final Logger logger = LoggerFactory.getLogger(CdlConcealBabysWall.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlConcealBabysWall() {
    }

    /**
     * Calculates the Concealing Baby Swallow pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Concealing Baby Swallow pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLCONCEALBABYSWALL(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLCONCEALBABYSWALL returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLCONCEALBABYSWALL returned error code: " + retCode);
        }
