...
/**
 * This class is a wrapper for the TA-Lib function MINUS_DI: Minus Directional Indicator.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class MinusDI {
    ...
    private static final Logger logger = LoggerFactory.getLogger(MinusDI.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Minus Directional Indicator of a given input series.
     *
     ...
     * @return a Result object containing the calculated Minus Directional Indicator
    ...
    public static Result execute(int startIdx, int endIdx, double[] high, double[] low, double[] close, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_MINUS_DI(startIdx, endIdx, high, low, close, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function MINUS_DI returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function MINUS_DI returned error code: " + retCode);
        }
