...
/**
 * This class is a wrapper for the TA-Lib function CCI: Commodity Channel Index.
 */
public class Cci {

    private static final Logger logger = LoggerFactory.getLogger(Cci.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] high, double[] low, double[] close, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CCI(startIdx, endIdx, high, low, close, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function CCI returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CCI returned error code: " + retCode);
        }
