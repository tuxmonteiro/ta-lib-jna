...
/**
 * This class is a wrapper for the TA-Lib function CDLKICKING: Kicking.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlKicking {

    private static final Logger logger = LoggerFactory.getLogger(CdlKicking.class);
    ...
    /**
     * Calculates the Kicking pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Kicking pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLKICKING(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLKICKING returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLKICKING returned error code: " + retCode);
        }
