...
/**
 * This class is a wrapper for the TA-Lib function TRANGE: True Range.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class TrueRange {
    ...
    private static final Logger logger = LoggerFactory.getLogger(TrueRange.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the True Range of a given input series.
     *
     ...
     * @return a Result object containing the calculated True Range
    ...
    public static Result execute(int startIdx, int endIdx, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_TRANGE(startIdx, endIdx, high, low, close, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function TRANGE returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function TRANGE returned error code: " + retCode);
        }
