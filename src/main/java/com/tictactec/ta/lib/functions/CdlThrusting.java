...
/**
 * This class is a wrapper for the TA-Lib function CDLTHRUSTING: Thrusting Pattern.
 */
public class CdlThrusting {

    private static final Logger logger = LoggerFactory.getLogger(CdlThrusting.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLTHRUSTING(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLTHRUSTING returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLTHRUSTING returned error code: " + retCode);
        }
