...
/**
 * This class is a wrapper for the TA-Lib function CDLSPINNINGTOP: Spinning Top.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlSpinningTop {

    private static final Logger logger = LoggerFactory.getLogger(CdlSpinningTop.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlSpinningTop() {
    }

    /**
     * Calculates the Spinning Top pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Spinning Top pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLSPINNINGTOP(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLSPINNINGTOP returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLSPINNINGTOP returned error code: " + retCode);
        }
