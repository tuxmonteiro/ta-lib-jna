...
/**
 * This class is a wrapper for the TA-Lib function CDLADVANCEBLOCK: Advance Block.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlAdvanceBlock {

    private static final Logger logger = LoggerFactory.getLogger(CdlAdvanceBlock.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlAdvanceBlock() {
    }

    /**
     * Calculates the Advance Block pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Advance Block pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLADVANCEBLOCK(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLADVANCEBLOCK returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLADVANCEBLOCK returned error code: " + retCode);
        }
