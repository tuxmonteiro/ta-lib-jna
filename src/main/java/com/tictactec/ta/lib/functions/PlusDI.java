...
/**
 * This class is a wrapper for the TA-Lib function PLUS_DI: Plus Directional Indicator.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class PlusDI {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PlusDI.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Plus Directional Indicator of a given input series.
     *
     ...
     * @return a Result object containing the calculated Plus Directional Indicator
    ...
    public static Result execute(int startIdx, int endIdx, double[] high, double[] low, double[] close, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_PLUS_DI(startIdx, endIdx, high, low, close, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function PLUS_DI returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function PLUS_DI returned error code: " + retCode);
        }
