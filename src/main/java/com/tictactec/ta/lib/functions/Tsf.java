...
/**
 * This class is a wrapper for the TA-Lib function TSF: Time Series Forecast.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Tsf {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Tsf.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Time Series Forecast of a given input series.
     *
     ...
     * @return a Result object containing the calculated TSF
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_TSF(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function TSF returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function TSF returned error code: " + retCode);
        }
