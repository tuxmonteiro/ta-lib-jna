...
public class StdDev {
    ...
    private static final Logger logger = LoggerFactory.getLogger(StdDev.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Standard Deviation of a given input series.
     *
     ...
     * @return a Result object containing the calculated Standard Deviation
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod, double optInDeviations) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        if (retCode != 0) {
            logger.error("TA-Lib function STDDEV returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function STDDEV returned error code: " + retCode);
        }
