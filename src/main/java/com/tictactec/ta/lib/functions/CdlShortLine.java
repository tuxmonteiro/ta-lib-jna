...
/**
 * This class is a wrapper for the TA-Lib function CDLSHORTLINE: Short Line Candle.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlShortLine {

    private static final Logger logger = LoggerFactory.getLogger(CdlShortLine.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlShortLine() {
    }

    /**
     * Calculates the Short Line Candle pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Short Line Candle pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLSHORTLINE(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLSHORTLINE returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLSHORTLINE returned error code: " + retCode);
        }
