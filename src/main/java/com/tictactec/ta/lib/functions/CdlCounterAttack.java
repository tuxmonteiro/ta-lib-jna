...
/**
 * This class is a wrapper for the TA-Lib function CDLCOUNTERATTACK: Counterattack.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlCounterAttack {

    private static final Logger logger = LoggerFactory.getLogger(CdlCounterAttack.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlCounterAttack() {
    }

    /**
     * Calculates the Counterattack pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Counterattack pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLCOUNTERATTACK(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLCOUNTERATTACK returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLCOUNTERATTACK returned error code: " + retCode);
        }
