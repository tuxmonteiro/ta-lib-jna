...
/**
 * This class is a wrapper for the TA-Lib function CDLDOJI: Doji.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlDoji {

    private static final Logger logger = LoggerFactory.getLogger(CdlDoji.class);
    ...
    /**
     * Calculates the Doji pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Doji pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLDOJI(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLDOJI returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLDOJI returned error code: " + retCode);
        }
