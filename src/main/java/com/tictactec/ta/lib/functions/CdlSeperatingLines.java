...
/**
 * This class is a wrapper for the TA-Lib function CDLSEPARATINGLINES: Separating Lines.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlSeperatingLines {

    private static final Logger logger = LoggerFactory.getLogger(CdlSeperatingLines.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlSeperatingLines() {
    }

    /**
     * Calculates the Separating Lines pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Separating Lines pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLSEPARATINGLINES(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLSEPARATINGLINES returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLSEPARATINGLINES returned error code: " + retCode);
        }
