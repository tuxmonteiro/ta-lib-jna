...
/**
 * This class is a wrapper for the TA-Lib function WMA: Weighted Moving Average.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Wma {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Wma.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Weighted Moving Average of a given input series.
     *
     ...
     * @return a Result object containing the calculated WMA
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_WMA(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function WMA returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function WMA returned error code: " + retCode);
        }
