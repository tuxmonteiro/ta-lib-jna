...
/**
 * This class is a wrapper for the TA-Lib function CDLMATCHINGLOW: Matching Low.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlMatchingLow {

    private static final Logger logger = LoggerFactory.getLogger(CdlMatchingLow.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlMatchingLow() {
    }

    /**
     * Calculates the Matching Low pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Matching Low pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLMATCHINGLOW(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLMATCHINGLOW returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLMATCHINGLOW returned error code: " + retCode);
        }
