...
/**
 * This class is a wrapper for the TA-Lib function CDLMORNINGSTAR: Morning Star.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlMorningStar {

    private static final Logger logger = LoggerFactory.getLogger(CdlMorningStar.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlMorningStar() {
    }

    /**
     * Calculates the Morning Star pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Morning Star pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close, double optInPenetration) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLMORNINGSTAR(startIdx, endIdx, open, high, low, close, optInPenetration, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLMORNINGSTAR returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLMORNINGSTAR returned error code: " + retCode);
        }
