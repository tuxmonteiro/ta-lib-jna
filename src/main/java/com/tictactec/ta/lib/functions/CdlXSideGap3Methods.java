...
/**
 * This class is a wrapper for the TA-Lib function CDLXSIDEGAP3METHODS: Upside/Downside Gap Three Methods.
 */
public class CdlXSideGap3Methods {

    private static final Logger logger = LoggerFactory.getLogger(CdlXSideGap3Methods.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLXSIDEGAP3METHODS(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLXSIDEGAP3METHODS returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLXSIDEGAP3METHODS returned error code: " + retCode);
        }
