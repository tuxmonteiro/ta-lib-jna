...
/**
 * This class is a wrapper for the TA-Lib function PLUS_DM: Plus Directional Movement.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class PlusDM {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PlusDM.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Plus Directional Movement of a given input series.
     *
     ...
     * @return a Result object containing the calculated Plus Directional Movement
    ...
    public static Result execute(int startIdx, int endIdx, double[] high, double[] low, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_PLUS_DM(startIdx, endIdx, high, low, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function PLUS_DM returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function PLUS_DM returned error code: " + retCode);
        }
