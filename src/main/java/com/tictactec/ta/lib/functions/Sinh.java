...
/**
 * This class is a wrapper for the TA-Lib function SINH: Vector Trigonometric Sinh.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Sinh {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Sinh.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the vector hyperbolic sine of a given input series.
     *
     ...
     * @return a Result object containing the calculated vector hyperbolic sine
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_SINH(startIdx, endIdx, inreal, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function SINH returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function SINH returned error code: " + retCode);
        }
