...
/**
 * This class is a wrapper for the TA-Lib function CDLSTALLEDPATTERN: Stalled Pattern.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlStalledPattern {

    private static final Logger logger = LoggerFactory.getLogger(CdlStalledPattern.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlStalledPattern() {
    }

    /**
     * Calculates the Stalled Pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Stalled Pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLSTALLEDPATTERN(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLSTALLEDPATTERN returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLSTALLEDPATTERN returned error code: " + retCode);
        }
