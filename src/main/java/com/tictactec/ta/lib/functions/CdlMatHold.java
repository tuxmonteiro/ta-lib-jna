...
/**
 * This class is a wrapper for the TA-Lib function CDLMATHOLD: Mat Hold Pattern.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlMatHold {

    private static final Logger logger = LoggerFactory.getLogger(CdlMatHold.class);
    ...
    /**
     * Calculates the Mat Hold Pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Mat Hold Pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close, double optInPenetration) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLMATHOLD(startIdx, endIdx, open, high, low, close, optInPenetration, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLMATHOLD returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLMATHOLD returned error code: " + retCode);
        }
