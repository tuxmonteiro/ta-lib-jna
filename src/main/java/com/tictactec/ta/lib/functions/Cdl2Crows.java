...
public class Cdl2Crows {
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDL2CROWS(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDL2CROWS returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDL2CROWS returned error code: " + retCode);
        }
