...
/**
 * This class is a wrapper for the TA-Lib function CEIL: Vector Ceil.
 */
public class Ceil {

    private static final Logger logger = LoggerFactory.getLogger(Ceil.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CEIL(startIdx, endIdx, inreal, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function CEIL returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CEIL returned error code: " + retCode);
        }
