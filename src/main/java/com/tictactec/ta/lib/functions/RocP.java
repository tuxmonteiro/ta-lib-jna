...
/**
 * This class is a wrapper for the TA-Lib function ROCP: Rate of change Percentage: (price-prevPrice)/prevPrice.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class RocP {
    ...
    private static final Logger logger = LoggerFactory.getLogger(RocP.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Rate of Change Percentage of a given input series.
     *
     ...
     * @return a Result object containing the calculated ROCP
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_ROCP(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function ROCP returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function ROCP returned error code: " + retCode);
        }
