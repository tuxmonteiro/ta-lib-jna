...
/**
 * This class is a wrapper for the TA-Lib function CDLLADDERBOTTOM: Ladder Bottom.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlLadderBottom {

    private static final Logger logger = LoggerFactory.getLogger(CdlLadderBottom.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlLadderBottom() {
    }

    /**
     * Calculates the Ladder Bottom pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Ladder Bottom pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLLADDERBOTTOM(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLLADDERBOTTOM returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLLADDERBOTTOM returned error code: " + retCode);
        }
