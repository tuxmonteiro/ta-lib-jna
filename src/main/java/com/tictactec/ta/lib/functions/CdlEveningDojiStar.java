...
/**
 * This class is a wrapper for the TA-Lib function CDLEVENINGDOJISTAR: Evening Doji Star.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlEveningDojiStar {

    private static final Logger logger = LoggerFactory.getLogger(CdlEveningDojiStar.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlEveningDojiStar() {
    }

    /**
     * Calculates the Evening Doji Star pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Evening Doji Star pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close, double optInPenetration) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLEVENINGDOJISTAR(startIdx, endIdx, open, high, low, close, optInPenetration, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLEVENINGDOJISTAR returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLEVENINGDOJISTAR returned error code: " + retCode);
        }
