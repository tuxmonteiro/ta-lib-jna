...
/**
 * This class is a wrapper for the TA-Lib function CDLLONGLEGGEDDOJI: Long Legged Doji.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlLongLeggedDoji {

    private static final Logger logger = LoggerFactory.getLogger(CdlLongLeggedDoji.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlLongLeggedDoji() {
    }

    /**
     * Calculates the Long Legged Doji pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Long Legged Doji pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLLONGLEGGEDDOJI(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLLONGLEGGEDDOJI returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLLONGLEGGEDDOJI returned error code: " + retCode);
        }
