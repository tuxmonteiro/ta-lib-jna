...
/**
 * This class is a wrapper for the TA-Lib function CDLABANDONEDBABY: Abandoned Baby.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlAbandonedBaby {

    private static final Logger logger = LoggerFactory.getLogger(CdlAbandonedBaby.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlAbandonedBaby() {
    }

    /**
     * Calculates the Abandoned Baby pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Abandoned Baby pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close, double optInPenetration) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLABANDONEDBABY(startIdx, endIdx, open, high, low, close, optInPenetration, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLABANDONEDBABY returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLABANDONEDBABY returned error code: " + retCode);
        }
