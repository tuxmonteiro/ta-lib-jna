...
/**
 * This class is a wrapper for the TA-Lib function CDLMARUBOZU: Marubozu.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlMarubozu {

    private static final Logger logger = LoggerFactory.getLogger(CdlMarubozu.class);
    ...
    /**
     * Calculates the Marubozu pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Marubozu pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLMARUBOZU(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLMARUBOZU returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLMARUBOZU returned error code: " + retCode);
        }
