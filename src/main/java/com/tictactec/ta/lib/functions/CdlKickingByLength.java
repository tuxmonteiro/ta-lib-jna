...
/**
 * This class is a wrapper for the TA-Lib function CDLKICKINGBYLENGTH: Kicking - bull/bear determined by the longer marubozu.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlKickingByLength {

    private static final Logger logger = LoggerFactory.getLogger(CdlKickingByLength.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlKickingByLength() {
    }

    /**
     * Calculates the Kicking - bull/bear determined by the longer marubozu pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Kicking - bull/bear determined by the longer marubozu pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLKICKINGBYLENGTH(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLKICKINGBYLENGTH returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLKICKINGBYLENGTH returned error code: " + retCode);
        }
