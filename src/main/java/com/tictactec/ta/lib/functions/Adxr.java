...
/**
 * This class is a wrapper for the TA-Lib function ADXR: Average Directional Movement Index Rating.
 */
public class Adxr {

    private static final Logger logger = LoggerFactory.getLogger(Adxr.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] high, double[] low, double[] close, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_ADXR(startIdx, endIdx, high, low, close, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function ADXR returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function ADXR returned error code: " + retCode);
        }
