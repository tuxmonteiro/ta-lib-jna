...
/**
 * This class is a wrapper for the TA-Lib function CDL3LINESTRIKE: Three-Line Strike .
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Cdl3LineStrike {

    private static final Logger logger = LoggerFactory.getLogger(Cdl3LineStrike.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Three-Line Strike pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Three-Line Strike pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDL3LINESTRIKE(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDL3LINESTRIKE returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDL3LINESTRIKE returned error code: " + retCode);
        }
