...
/**
 * This class is a wrapper for the TA-Lib function WCLPRICE: Weighted Close Price.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class WclPrice {
    ...
    private static final Logger logger = LoggerFactory.getLogger(WclPrice.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Weighted Close Price of a given input series.
     *
     ...
     * @return a Result object containing the calculated Weighted Close Price
    ...
    public static Result execute(int startIdx, int endIdx, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_WCLPRICE(startIdx, endIdx, high, low, close, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function WCLPRICE returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function WCLPRICE returned error code: " + retCode);
        }
