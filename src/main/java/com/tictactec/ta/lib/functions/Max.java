...
/**
 * This class is a wrapper for the TA-Lib function MAX: Highest value over a specified period.
 */
public class Max {
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_MAX(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function MAX returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function MAX returned error code: " + retCode);
        }
