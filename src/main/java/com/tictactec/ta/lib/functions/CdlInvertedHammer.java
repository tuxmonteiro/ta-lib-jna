...
/**
 * This class is a wrapper for the TA-Lib function CDLINVERTEDHAMMER: Inverted Hammer.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlInvertedHammer {

    private static final Logger logger = LoggerFactory.getLogger(CdlInvertedHammer.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlInvertedHammer() {
    }

    /**
     * Calculates the Inverted Hammer pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Inverted Hammer pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLINVERTEDHAMMER(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLINVERTEDHAMMER returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLINVERTEDHAMMER returned error code: " + retCode);
        }
