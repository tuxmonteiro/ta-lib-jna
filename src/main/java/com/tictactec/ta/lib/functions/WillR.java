...
public class WillR {
    ...
    /**
     * Calculates the Williams' %%R of a given input series.
     *
     ...
     * @return a Result object containing the calculated Williams' %%R
    ...
    public static Result execute(int startIdx, int endIdx, double[] high, double[] low, double[] close, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_WILLR(startIdx, endIdx, high, low, close, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
