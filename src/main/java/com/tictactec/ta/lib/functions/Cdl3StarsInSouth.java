...
/**
 * This class is a wrapper for the TA-Lib function CDL3STARSINSOUTH: Three Stars In The South.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Cdl3StarsInSouth {

    private static final Logger logger = LoggerFactory.getLogger(Cdl3StarsInSouth.class);
    private static final TALib taLib = TALib.INSTANCE;

    private Cdl3StarsInSouth() {
    }

    /**
     * Calculates the Three Stars In The South pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Three Stars In The South pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDL3STARSINSOUTH(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDL3STARSINSOUTH returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDL3STARSINSOUTH returned error code: " + retCode);
        }
