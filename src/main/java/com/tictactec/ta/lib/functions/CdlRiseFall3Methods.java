...
/**
 * This class is a wrapper for the TA-Lib function CDLRISEFALL3METHODS: Rising/Falling Three Methods.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlRiseFall3Methods {

    private static final Logger logger = LoggerFactory.getLogger(CdlRiseFall3Methods.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlRiseFall3Methods() {
    }

    /**
     * Calculates the Rising/Falling Three Methods pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Rising/Falling Three Methods pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLRISEFALL3METHODS(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLRISEFALL3METHODS returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLRISEFALL3METHODS returned error code: " + retCode);
        }
