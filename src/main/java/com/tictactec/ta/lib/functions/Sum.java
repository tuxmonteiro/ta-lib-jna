...
/**
 * This class is a wrapper for the TA-Lib function SUM: Summation.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Sum {
    ...
    /**
     * Calculates the summation of a given input series.
     *
     ...
     * @return a Result object containing the calculated summation
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_SUM(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function SUM returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function SUM returned error code: " + retCode);
        }
