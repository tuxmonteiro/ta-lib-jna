...
/**
 * This class is a wrapper for the TA-Lib function CDLHIGHWAVE: High-Wave Candle.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlHignWave {

    private static final Logger logger = LoggerFactory.getLogger(CdlHignWave.class);
    ...
    /**
     * Calculates the High-Wave Candle pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated High-Wave Candle pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLHIGHWAVE(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLHIGHWAVE returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLHIGHWAVE returned error code: " + retCode);
        }
