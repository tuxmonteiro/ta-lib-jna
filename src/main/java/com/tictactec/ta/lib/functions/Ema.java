...
/**
 * This class is a wrapper for the TA-Lib function EMA: Exponential Moving Average.
 */
public class Ema {
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_EMA(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function EMA returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function EMA returned error code: " + retCode);
        }
