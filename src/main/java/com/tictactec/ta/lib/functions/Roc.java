...
/**
 * This class is a wrapper for the TA-Lib function ROC: Rate of change : ((price/prevPrice)-1)*100.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Roc {
    ...
    /**
     * Calculates the Rate of Change of a given input series.
     *
     ...
     * @return a Result object containing the calculated ROC
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_ROC(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function ROC returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function ROC returned error code: " + retCode);
        }
