...
/**
 * This class is a wrapper for the TA-Lib function CDL3OUTSIDE: Three Outside Up/Down.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Cdl3Outside {

    private static final Logger logger = LoggerFactory.getLogger(Cdl3Outside.class);
    ...
    /**
     * Calculates the Three Outside Up/Down pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Three Outside Up/Down pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDL3OUTSIDE(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDL3OUTSIDE returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDL3OUTSIDE returned error code: " + retCode);
        }
