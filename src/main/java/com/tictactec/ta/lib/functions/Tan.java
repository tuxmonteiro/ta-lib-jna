...
/**
 * This class is a wrapper for the TA-Lib function TAN: Vector Trigonometric Tan.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Tan {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Tan.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the vector tangent of a given input series.
     *
     ...
     * @return a Result object containing the calculated vector tangent
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_TAN(startIdx, endIdx, inreal, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function TAN returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function TAN returned error code: " + retCode);
        }
