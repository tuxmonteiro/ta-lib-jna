...
/**
 * This class is a wrapper for the TA-Lib function LINEARREG_ANGLE: Linear Regression Angle.
 */
public class LinearRegAngle {

    private static final Logger logger = LoggerFactory.getLogger(LinearRegAngle.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_LINEARREG_ANGLE(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function LINEARREG_ANGLE returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function LINEARREG_ANGLE returned error code: " + retCode);
        }
