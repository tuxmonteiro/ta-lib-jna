...
/**
 * This class is a wrapper for the TA-Lib function CDLHOMINGPIGEON: Homing Pigeon.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlHomingPigeon {

    private static final Logger logger = LoggerFactory.getLogger(CdlHomingPigeon.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlHomingPigeon() {
    }

    /**
     * Calculates the Homing Pigeon pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Homing Pigeon pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLHOMINGPIGEON(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLHOMINGPIGEON returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLHOMINGPIGEON returned error code: " + retCode);
        }
