...
/**
 * This class is a wrapper for the TA-Lib function TRIMA: Triangular Moving Average.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Trima {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Trima.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Triangular Moving Average of a given input series.
     *
     ...
     * @return a Result object containing the calculated TRIMA
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_TRIMA(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function TRIMA returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function TRIMA returned error code: " + retCode);
        }
