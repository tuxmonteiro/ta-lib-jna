...
/**
 * This class is a wrapper for the TA-Lib function CDLGAPSIDESIDEWHITE: Up/Down-side Gap Three Methods.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlGapSideSideWhite {

    private static final Logger logger = LoggerFactory.getLogger(CdlGapSideSideWhite.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlGapSideSideWhite() {
    }

    /**
     * Calculates the Up/Down-side Gap Three Methods pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Up/Down-side Gap Three Methods pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLGAPSIDESIDEWHITE(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLGAPSIDESIDEWHITE returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLGAPSIDESIDEWHITE returned error code: " + retCode);
        }
