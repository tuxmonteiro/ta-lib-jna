...
/**
 * This class is a wrapper for the TA-Lib function CDLDRAGONFLYDOJI: Dragonfly Doji.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlDragonflyDoji {

    private static final Logger logger = LoggerFactory.getLogger(CdlDragonflyDoji.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlDragonflyDoji() {
    }

    /**
     * Calculates the Dragonfly Doji pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Dragonfly Doji pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLDRAGONFLYDOJI(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLDRAGONFLYDOJI returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLDRAGONFLYDOJI returned error code: " + retCode);
        }
