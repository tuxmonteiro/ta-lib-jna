...
public class CdlEveningStar {
    ...
    private CdlEveningStar() {
    }

    /**
     * Calculates the Evening Star pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Evening Star pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close, double optInPenetration) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLEVENINGSTAR(startIdx, endIdx, open, high, low, close, optInPenetration, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
