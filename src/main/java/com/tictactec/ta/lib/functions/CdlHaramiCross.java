...
/**
 * This class is a wrapper for the TA-Lib function CDLHARAMICROSS: Harami Cross Pattern.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlHaramiCross {

    private static final Logger logger = LoggerFactory.getLogger(CdlHaramiCross.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlHaramiCross() {
    }

    /**
     * Calculates the Harami Cross Pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Harami Cross Pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLHARAMICROSS(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLHARAMICROSS returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLHARAMICROSS returned error code: " + retCode);
        }
