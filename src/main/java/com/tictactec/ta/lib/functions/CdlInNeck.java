...
/**
 * This class is a wrapper for the TA-Lib function CDLINNECK: In-Neck Pattern.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlInNeck {

    private static final Logger logger = LoggerFactory.getLogger(CdlInNeck.class);
    ...
    /**
     * Calculates the In-Neck Pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated In-Neck Pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLINNECK(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLINNECK returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLINNECK returned error code: " + retCode);
        }
