...
/**
 * This class is a wrapper for the TA-Lib function CDLLONGLINE: Long Line Candle.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlLongLine {

    private static final Logger logger = LoggerFactory.getLogger(CdlLongLine.class);
    ...
    /**
     * Calculates the Long Line Candle pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Long Line Candle pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLLONGLINE(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLLONGLINE returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLLONGLINE returned error code: " + retCode);
        }
