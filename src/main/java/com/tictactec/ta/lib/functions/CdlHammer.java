...
/**
 * This class is a wrapper for the TA-Lib function CDLHAMMER: Hammer.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlHammer {

    private static final Logger logger = LoggerFactory.getLogger(CdlHammer.class);
    ...
    /**
     * Calculates the Hammer pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Hammer pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLHAMMER(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLHAMMER returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLHAMMER returned error code: " + retCode);
        }
