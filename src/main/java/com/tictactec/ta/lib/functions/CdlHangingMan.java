...
/**
 * This class is a wrapper for the TA-Lib function CDLHANGINGMAN: Hanging Man.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlHangingMan {

    private static final Logger logger = LoggerFactory.getLogger(CdlHangingMan.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlHangingMan() {
    }

    /**
     * Calculates the Hanging Man pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Hanging Man pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLHANGINGMAN(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLHANGINGMAN returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLHANGINGMAN returned error code: " + retCode);
        }
