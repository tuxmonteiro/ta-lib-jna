...
/**
 * This class is a wrapper for the TA-Lib function SIN: Vector Trigonometric Sin.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Sin {
    ...
    /**
     * Calculates the vector sine of a given input series.
     *
     ...
     * @return a Result object containing the calculated vector sine
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_SIN(startIdx, endIdx, inreal, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function SIN returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function SIN returned error code: " + retCode);
        }
