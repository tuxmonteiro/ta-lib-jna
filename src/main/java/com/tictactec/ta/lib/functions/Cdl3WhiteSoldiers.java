...
/**
 * This class is a wrapper for the TA-Lib function CDL3WHITESOLDIERS: Three Advancing White Soldiers.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Cdl3WhiteSoldiers {

    private static final Logger logger = LoggerFactory.getLogger(Cdl3WhiteSoldiers.class);
    private static final TALib taLib = TALib.INSTANCE;

    private Cdl3WhiteSoldiers() {
    }

    /**
     * Calculates the Three Advancing White Soldiers pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Three Advancing White Soldiers pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDL3WHITESOLDIERS(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDL3WHITESOLDIERS returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDL3WHITESOLDIERS returned error code: " + retCode);
        }
