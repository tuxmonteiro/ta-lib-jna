...
/**
 * This class is a wrapper for the TA-Lib function CDLUNIQUE3RIVER: Unique 3 River.
 */
public class CdlUnique3River {

    private static final Logger logger = LoggerFactory.getLogger(CdlUnique3River.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLUNIQUE3RIVER(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLUNIQUE3RIVER returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLUNIQUE3RIVER returned error code: " + retCode);
        }
