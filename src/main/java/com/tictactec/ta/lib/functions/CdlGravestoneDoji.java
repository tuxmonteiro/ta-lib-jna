...
public class CdlGravestoneDoji {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CdlGravestoneDoji.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlGravestoneDoji() {
    }

    /**
     * Calculates the Gravestone Doji pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Gravestone Doji pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        if (retCode != 0) {
            logger.error("TA-Lib function CDLGRAVESTONEDOJI returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLGRAVESTONEDOJI returned error code: " + retCode);
        }
