...
/**
 * This class is a wrapper for the TA-Lib function CDLONNECK: On-Neck Pattern.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlOnNeck {

    private static final Logger logger = LoggerFactory.getLogger(CdlOnNeck.class);
    ...
    /**
     * Calculates the On-Neck Pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated On-Neck Pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLONNECK(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLONNECK returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLONNECK returned error code: " + retCode);
        }
