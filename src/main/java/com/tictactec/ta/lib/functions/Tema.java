...
/**
 * This class is a wrapper for the TA-Lib function TEMA: Triple Exponential Moving Average.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Tema {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Tema.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Triple Exponential Moving Average of a given input series.
     *
     ...
     * @return a Result object containing the calculated TEMA
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_TEMA(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function TEMA returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function TEMA returned error code: " + retCode);
        }
