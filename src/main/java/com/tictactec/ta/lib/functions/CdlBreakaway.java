...
/**
 * This class is a wrapper for the TA-Lib function CDLBREAKAWAY: Breakaway.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlBreakaway {

    private static final Logger logger = LoggerFactory.getLogger(CdlBreakaway.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlBreakaway() {
    }

    /**
     * Calculates the Breakaway pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Breakaway pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLBREAKAWAY(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLBREAKAWAY returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLBREAKAWAY returned error code: " + retCode);
        }
