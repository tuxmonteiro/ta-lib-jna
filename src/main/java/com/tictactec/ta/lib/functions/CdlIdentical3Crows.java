...
/**
 * This class is a wrapper for the TA-Lib function CDLIDENTICAL3CROWS: Identical Three Crows.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlIdentical3Crows {

    private static final Logger logger = LoggerFactory.getLogger(CdlIdentical3Crows.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlIdentical3Crows() {
    }

    /**
     * Calculates the Identical Three Crows pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Identical Three Crows pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLIDENTICAL3CROWS(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLIDENTICAL3CROWS returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLIDENTICAL3CROWS returned error code: " + retCode);
        }
