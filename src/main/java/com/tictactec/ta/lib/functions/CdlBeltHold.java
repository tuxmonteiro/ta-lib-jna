...
/**
 * This class is a wrapper for the TA-Lib function CDLBELTHOLD: Belt-hold.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlBeltHold {

    private static final Logger logger = LoggerFactory.getLogger(CdlBeltHold.class);
    ...
    /**
     * Calculates the Belt-hold pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Belt-hold pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLBELTHOLD(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLBELTHOLD returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLBELTHOLD returned error code: " + retCode);
        }
