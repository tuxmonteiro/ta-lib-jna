...
/**
 * This class is a wrapper for the TA-Lib function CDLDOJISTAR: Doji Star.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlDojiStar {

    private static final Logger logger = LoggerFactory.getLogger(CdlDojiStar.class);
    ...
    /**
     * Calculates the Doji Star pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Doji Star pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLDOJISTAR(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLDOJISTAR returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLDOJISTAR returned error code: " + retCode);
        }
