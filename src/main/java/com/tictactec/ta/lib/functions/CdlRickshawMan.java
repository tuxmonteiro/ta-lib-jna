...
/**
 * This class is a wrapper for the TA-Lib function CDLRICKSHAWMAN: Rickshaw Man.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlRickshawMan {

    private static final Logger logger = LoggerFactory.getLogger(CdlRickshawMan.class);
    private static final TALib taLib = TALib.INSTANCE;

    private CdlRickshawMan() {
    }

    /**
     * Calculates the Rickshaw Man pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Rickshaw Man pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLRICKSHAWMAN(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLRICKSHAWMAN returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLRICKSHAWMAN returned error code: " + retCode);
        }
