...
/**
 * This class is a wrapper for the TA-Lib function CDLHARAMI: Harami Pattern.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class CdlHarami {

    private static final Logger logger = LoggerFactory.getLogger(CdlHarami.class);
    ...
    /**
     * Calculates the Harami Pattern of a given input series.
     *
     ...
     * @return a Result object containing the calculated Harami Pattern
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLHARAMI(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLHARAMI returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLHARAMI returned error code: " + retCode);
        }
