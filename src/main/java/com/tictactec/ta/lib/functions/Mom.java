...
/**
 * This class is a wrapper for the TA-Lib function MOM: Momentum.
 *
 * @author fibonsai
 * @since 0.6.4
 */
public class Mom {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Mom.class);
    private static final TALib taLib = TALib.INSTANCE;

    /**
     * Calculates the Momentum of a given input series.
     *
     ...
     * @return a Result object containing the calculated Momentum
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_MOM(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function MOM returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function MOM returned error code: " + retCode);
        }
