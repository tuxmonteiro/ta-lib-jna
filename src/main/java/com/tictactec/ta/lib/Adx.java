...
/**
 * This class is a wrapper for the TA-Lib function ADX: Average Directional Movement Index.
 */
public class Adx {

    private static final Logger logger = LoggerFactory.getLogger(Adx.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] high, double[] low, double[] close, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_ADX(startIdx, endIdx, high, low, close, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function ADX returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function ADX returned error code: " + retCode);
        }
