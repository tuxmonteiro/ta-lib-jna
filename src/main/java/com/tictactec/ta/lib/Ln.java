...
/**
 * This class is a wrapper for the TA-Lib function LN: Vector Log Natural.
 */
public class Ln {

    private static final Logger logger = LoggerFactory.getLogger(Ln.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_LN(startIdx, endIdx, inreal, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function LN returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function LN returned error code: " + retCode);
        }
