...
/**
 * This class is a wrapper for the TA-Lib function CDLUPSIDEGAP2CROWS: Upside Gap Two Crows.
 */
public class CdlUpsideGap2Crows {

    private static final Logger logger = LoggerFactory.getLogger(CdlUpsideGap2Crows.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLUPSIDEGAP2CROWS(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLUPSIDEGAP2CROWS returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLUPSIDEGAP2CROWS returned error code: " + retCode);
        }
