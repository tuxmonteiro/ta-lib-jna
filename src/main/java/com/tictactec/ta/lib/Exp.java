...
/**
 * This class is a wrapper for the TA-Lib function EXP: Vector Arithmetic Exp.
 */
public class Exp {
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_EXP(startIdx, endIdx, inreal, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function EXP returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function EXP returned error code: " + retCode);
        }
