...
/**
 * This class is a wrapper for the TA-Lib function DEMA: Double Exponential Moving Average.
 */
public class Dema {

    private static final Logger logger = LoggerFactory.getLogger(Dema.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_DEMA(startIdx, endIdx, inreal, optInTimePeriod, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function DEMA returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function DEMA returned error code: " + retCode);
        }
