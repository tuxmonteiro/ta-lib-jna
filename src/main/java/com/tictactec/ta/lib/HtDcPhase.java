...
/**
 * This class is a wrapper for the TA-Lib function HT_DCPHASE: Hilbert Transform - Dominant Cycle Phase.
 */
public class HtDcPhase {

    private static final Logger logger = LoggerFactory.getLogger(HtDcPhase.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_HT_DCPHASE(startIdx, endIdx, inreal, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function HT_DCPHASE returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function HT_DCPHASE returned error code: " + retCode);
        }
