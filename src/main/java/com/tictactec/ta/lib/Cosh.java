...
/**
 * This class is a wrapper for the TA-Lib function COSH: Vector Trigonometric Cosh.
 */
public class Cosh {

    private static final Logger logger = LoggerFactory.getLogger(Cosh.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_COSH(startIdx, endIdx, inreal, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function COSH returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function COSH returned error code: " + retCode);
        }
