...
/**
 * This class is a wrapper for the TA-Lib function FLOOR: Vector Floor.
 */
public class Floor {
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_FLOOR(startIdx, endIdx, inreal, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function FLOOR returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function FLOOR returned error code: " + retCode);
        }
