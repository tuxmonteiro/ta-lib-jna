...
/**
 * This class is a wrapper for the TA-Lib function HT_TRENDLINE: Hilbert Transform - Instantaneous Trendline.
 */
public class HtTrendline {

    private static final Logger logger = LoggerFactory.getLogger(HtTrendline.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_HT_TRENDLINE(startIdx, endIdx, inreal, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function HT_TRENDLINE returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function HT_TRENDLINE returned error code: " + retCode);
        }
