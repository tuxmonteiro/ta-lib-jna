...
/**
 * This class is a wrapper for the TA-Lib function ADD: Vector Arithmetic Add.
 */
public class Add {
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal0, double[] inreal1) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_ADD(startIdx, endIdx, inreal0, inreal1, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function ADD returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function ADD returned error code: " + retCode);
        }
