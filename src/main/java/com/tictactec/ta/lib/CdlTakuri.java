...
/**
 * This class is a wrapper for the TA-Lib function CDLTAKURI: Takuri (Dragonfly Doji with very long lower shadow).
 */
public class CdlTakuri {

    private static final Logger logger = LoggerFactory.getLogger(CdlTakuri.class);
    ...
    public static Result execute(int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_CDLTAKURI(startIdx, endIdx, open, high, low, close, outBegIdx, outNBElement, outInteger);
        if (retCode != 0) {
            logger.error("TA-Lib function CDLTAKURI returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function CDLTAKURI returned error code: " + retCode);
        }
