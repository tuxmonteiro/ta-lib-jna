...
public class Variance {
    ...
    public static Result execute(int startIdx, int endIdx, double[] inreal, int optInTimePeriod, double optInDeviations) throws ArithmeticException, IndexOutOfBoundsException {
        ...
        int retCode = taLib.TA_VAR(startIdx, endIdx, inreal, optInTimePeriod, optInDeviations, outBegIdx, outNBElement, outReal);
        if (retCode != 0) {
            logger.error("TA-Lib function VAR returned error code: {}", retCode);
            throw new ArithmeticException("TA-Lib function VAR returned error code: " + retCode);
        }
